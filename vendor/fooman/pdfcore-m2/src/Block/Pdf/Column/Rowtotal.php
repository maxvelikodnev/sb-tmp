...
class Rowtotal extends \Fooman\PdfCore\Block\Pdf\Column implements \Fooman\PdfCore\Block\Pdf\ColumnInterface
{
    ...
    public function getRowTotal($row)
    {
        ...
        if ($this->getUseOrderCurrency()) {
            if ($displayRowTotal) {
                return [['currency' => $this->getCurrencyCode(), 'amount' => $displayRowTotal]];
            }
            return $displayRowTotal;
        }
        if ($displayBaseRowTotal) {
            return [['currency' => $this->getBaseCurrencyCode(), 'amount' => $displayBaseRowTotal]];
        }
