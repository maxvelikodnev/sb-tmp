...
class Totals extends \Fooman\PdfCore\Block\Pdf\Block
{
    ...
    {
        $this->getOrder()->setOrderId($this->getOrder()->getId());
    ...
    private function getFixedTaxTotals($total)
    {
        ...
        if (empty($totalsReturn) && $this->getSalesObject()->getTaxAmount()) {
            //Fix for migrated orders with missing tax breakdown
            $totalsReturn[] = [
                'label' => __('Tax') . ':',
                'amount' => $this->getAmountPrefix()
                    . $this->getOrder()->getOrderCurrency()->formatTxt($this->getSalesObject()->getTaxAmount()),
                'base_amount' => $this->getAmountPrefix()
                    . $this->getOrder()->getBaseCurrency()->formatTxt($this->getSalesObject()->getBaseTaxAmount())
            ];
        }
        return $totalsReturn;
