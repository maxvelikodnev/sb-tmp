...
abstract class AbstractSalesDocument extends \Fooman\PdfCore\Block\Pdf\DocumentRenderer
{
    ...
    const XML_PATH_DISPLAY_CONFIG_CHILDREN = 'sales_pdf/all/display_configurable_child_attributes';
    ...
    {
        /** @var \Fooman\PdfCustomiser\Block\Label $block */
        $block = $this->getLayout()->createBlock(
            \Fooman\PdfCustomiser\Block\IntegratedLabel::class,
            'pdfcustomiser.' . strtolower($side) . uniqid()
        );
        ...
        $block->setBillingAddress($this->getBillingAddress());
        $block->setShippingAddress($this->getShippingAddress());
        $block->setSalesObject($this->getSalesObject());
        $block->setDesign($this->getDesign());
        $block->setTemplate($this->templateFileDecider->pick($this->getDesign(), $side));
        return $block->toHtml();
    }
    ...
    public function getCommentsBlock()
    {
        ...
        if ($printCommentsConfig) {
            $comments = $this->prepareOrderComments($salesObject, $printCommentsConfig, $comments);
        }
        $customComments = [];
        if ($this->getOrder()->getCustomerNote()) {
            $customComments[] = ['label' => '', 'value' => $this->getOrder()->getCustomerNote()];
        }
        $transport = new \Magento\Framework\DataObject(['custom_comments' => $customComments]);
        $this->eventManager->dispatch(
            'fooman_pdfcustomiser_collect_custom_comments',
            [
                'order' => $this->getOrder(),
        ...
        if (!empty($comments) || !empty($transport->getCustomComments())) {
            $block = $this->getLayout()->createBlock(
                \Fooman\PdfCustomiser\Block\Comments::class,
                'pdfcustomiser.comments' . uniqid(),
                ['data' => ['comments' => $comments, 'custom_comments'=> $transport->getCustomComments()]]
            );
    ...
    /**
     * Remove some fields on bundles, add in product attribute values
     *
     * @param $item
     *
     * @return mixed
    ...
    protected function addProductAttributeValues(\Magento\Sales\Api\Data\OrderItemInterface $orderItem, $item)
    {
        ...
        $displayConfigChild = $this->_scopeConfig->getValue(
            self::XML_PATH_DISPLAY_CONFIG_CHILDREN,
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE,
            $this->getStoreId()
        );
        if ($displayConfigChild
        ...
        foreach ($productAttributes as $productAttribute) {
            ...
            if ($productAttribute === 'quantity_and_stock_status') {
            ...
            } elseif ($productAttribute === 'category_ids') {
                $categoryNames = [];
                ...
                $value = implode('<br/>', $categoryNames);
            } else {
                ...
                if ($attribute->getSourceModel()) {
                    ...
                    $values = [];
                    $optIds = explode(',', $product->getData($productAttribute));
                    foreach ($optIds as $optId) {
                        $values[] = $product->getAttributeText($optId);
                    }
    ...
    /**
     * @param       $salesObject
     * @param       $printCommentsConfig
     * @param array $comments
     *
     * @return array
     */
    private function prepareOrderComments($salesObject, $printCommentsConfig, array $comments)
    {
        ...
        } elseif ($salesObject->getCommentsCollection(true)) {
            $commentObject = [];
            switch ($printCommentsConfig) {
                case \Fooman\PdfCustomiser\Model\System\PrintComments::PRINT_ALL:
    ...
    public function getGroupText($groupId)
    {
        $content = $this->_scopeConfig->getValue(
            static::XML_PATH_GROUPTEXT,
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE,
            $this->getStoreId()
        );
        if (!$content) {
        ...
        foreach ($content as $groupContent) {
            if ($groupContent['group_id'] === $groupId) {
                return $this->processCustomVars(
